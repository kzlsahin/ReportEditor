...
class ImgLabel extends HTMLElement {
    ...
    attributeChangedCallback(name, oldValue, newValue) {
        if (name === "label-number") {
            ...
            let textNode = this.shadowRoot.querySelector('SLOT').assignedElements()[0];
            textNode.setAttribute("contenteditable", newValue);
        }
    }
    updatePrefix() {
...
class IndexList extends HTMLElement {
    ...
    generate() {
        ...
        for (let heading of headings) {
            let level = heading.secLevel;
            if (level > this.tocDepth) {
                continue;
            }
            let content = heading.toString();
...
//# sourceMappingURL=EditorComponents.js.map